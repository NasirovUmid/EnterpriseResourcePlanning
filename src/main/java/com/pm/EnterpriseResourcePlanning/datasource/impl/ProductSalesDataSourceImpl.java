...
public class ProductSalesDataSourceImpl extends MessageAlertDataSource implements ProductSalesDataSource {
    ...
    public ProductSalesDataSourceImpl(AlertSystemDao alertSystemDao, ProductMapper productMapper, ProductSalesDao productSalesDao) {
    ...
    public void saveProductSales(UUID productId, UUID salesId, Integer quantity, Double price) {
        execute(() -> productSalesDao.saveProductSales(productId, salesId, quantity, price));
    }
    ...
    @Override
    public List<ProductsEntity> findAllByIds(Set<UUID> ids) {
        return execute(() -> productSalesDao.findAll(ids));
    }
